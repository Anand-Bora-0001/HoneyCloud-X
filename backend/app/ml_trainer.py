"""
HoneyCloud ML Training Service
...
class MLTrainingService:
    ...
    async def _get_training_data(self, organization_id: Optional[int] = None) -> List[Dict]:
        ...
        try:
            ...
            if organization_id:
                query = query.filter(AttackEvent.organization_id == organization_id)
            
            # Get events from last 30 days
            thirty_days_ago = datetime.now() - timedelta(days=30)
            query = query.filter(AttackEvent.timestamp >= thirty_days_ago)
            
            # Order by timestamp and limit
            events = query.order_by(AttackEvent.timestamp.desc()).limit(1000).all()
    ...
    async def _get_recent_events(self, limit: int = 100) -> List[Dict]:
        ...
        try:
            ...
            events = db.query(AttackEvent).filter(
                AttackEvent.timestamp >= recent_time
