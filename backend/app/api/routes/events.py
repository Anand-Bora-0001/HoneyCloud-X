...
async def ingest_event(event: Dict, request: Request):
    ...
    Used by Flask honeypot / external honeypots to send attack data.
    """
        ...
        try:
            ...
            if not org:
                org = Organization(
                    ...
                    email="demo@honeycloud.local", plan="free", is_trial=True
                )
                db.add(org)
                db.commit()
                db.refresh(org)
...
def get_events(
    ...
    try:
        ...
        query = db.query(AttackEvent).filter(AttackEvent.organization_id == user.organization_id)
        if service:
...
def clear_events(
    ...
    try:
        ...
        deleted_count = db.query(AttackEvent).filter(
...
async def event_stream(current_user: dict = Depends(get_current_user)):
    ...
    async def event_generator():
        ...
        while True:
            if len(attack_events) > last_id:
                for event in attack_events[last_id:]:
...
async def simulate_attacks(
    ...
    """Manually trigger attack simulation — captures REAL client IP"""
    ...
    for i in range(count):
        ...
        if new_event['severity'] in ['CRITICAL', 'HIGH']:
            try:
                ...
                handle_attack_event(new_event, db)
            except Exception:
                pass
    ...
    return {
        "status": "success",
        "message": f"Generated {count} attacks from {client_ip}",
