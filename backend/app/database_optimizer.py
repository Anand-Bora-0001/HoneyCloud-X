...
from .models import AttackEvent, Organization, User, Service
...
class IndexManager:
    ...
    def __init__(self, engine: Engine):
        ...
        self.recommended_indexes = [
            ...
            IndexInfo("attack_events", "idx_attack_events_timestamp", ["timestamp"], False),
            IndexInfo("attack_events", "idx_attack_events_source_ip", ["source_ip"], False),
            IndexInfo("attack_events", "idx_attack_events_severity", ["severity"], False),
            IndexInfo("attack_events", "idx_attack_events_service_name", ["service_name"], False),
            IndexInfo("attack_events", "idx_attack_events_org_timestamp", ["organization_id", "timestamp"], False),
            IndexInfo("attack_events", "idx_attack_events_severity_timestamp", ["severity", "timestamp"], False),
            IndexInfo("attack_events", "idx_attack_events_ip_service", ["source_ip", "service_name"], False),
    ...
    def _analyze_table_structure(self) -> Dict[str, Any]:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                tables = ['attack_events', 'users', 'organizations', 'services']
