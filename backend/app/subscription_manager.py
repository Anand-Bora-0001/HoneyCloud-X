...
class SubscriptionManager:
    ...
    def get_usage_stats(db: Session, organization: Organization) -> Dict:
        ...
        from .models import Service, AttackEvent
        ...
        events_this_month = db.query(AttackEvent).filter(
            AttackEvent.organization_id == organization.id,
            extract('month', AttackEvent.timestamp) == current_month,
            extract('year', AttackEvent.timestamp) == current_year
        ).count()
