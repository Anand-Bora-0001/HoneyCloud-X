...
Monitors ALL types of attacks: Honeypot services + Demo E-commerce + Web applications
"""
...
def print_banner():
    ...
    print("\n" + "="*70)
    print("🎯 HoneyCloud COMPREHENSIVE Attack Monitoring Suite")
    print("   Monitors ALL attack types: Honeypots + Web Apps + E-commerce")
    print("="*70)
...
def clear_database():
    """Clear the HoneyCloud database for fresh start"""
    print("🗑️  Clearing HoneyCloud database...")
...
def login_to_honeycloud():
    """Login to HoneyCloud and get token"""
    print("🔐 Authenticating with HoneyCloud...")
...
def simulate_ssh_honeypot_attacks():
    ...
    for i, attack in enumerate(ssh_attacks):
        ...
        send_attack_to_honeycloud(attack_data, f"SSH Attack {i+1}")
        time.sleep(ATTACK_DELAY)

def simulate_ftp_honeypot_attacks():
    """Simulate FTP honeypot attacks"""
    ...
    for i, attack in enumerate(ftp_attacks):
        ...
        send_attack_to_honeycloud(attack_data, f"FTP Attack {i+1}")
        time.sleep(ATTACK_DELAY)

def simulate_http_honeypot_attacks():
    """Simulate HTTP honeypot attacks"""
    ...
    for i, attack in enumerate(http_attacks):
        ...
        send_attack_to_honeycloud(attack_data, f"HTTP Attack {i+1}")
        time.sleep(ATTACK_DELAY)

def simulate_telnet_honeypot_attacks():
    """Simulate TELNET honeypot attacks"""
    ...
    for i, attack in enumerate(telnet_attacks):
        ...
        send_attack_to_honeycloud(attack_data, f"TELNET Attack {i+1}")
        time.sleep(ATTACK_DELAY)
...
def simulate_demo_ecommerce_attacks():
    ...
    for i in range(6):
        ...
        try:
            ...
            # Also send to HoneyCloud for monitoring
            attack_data = {
            ...
            send_attack_to_honeycloud(attack_data, f"E-commerce Login Attack {i+1}")
...
def simulate_sql_injection_attacks():
    ...
    for i, attack in enumerate(sql_payloads):
        ...
        send_attack_to_honeycloud(attack_data, f"SQL Injection {i+1}")
        time.sleep(ATTACK_DELAY)
...
def simulate_directory_traversal_attacks():
    ...
    for i, attack in enumerate(traversal_payloads):
        ...
        send_attack_to_honeycloud(attack_data, f"Directory Traversal {i+1}")
        time.sleep(ATTACK_DELAY)

def send_attack_to_honeycloud(attack_data, attack_name):
    """Send attack data to HoneyCloud"""
    try:
        response = requests.post(
...
def check_comprehensive_results(token):
    """Check all attack results in HoneyCloud"""
    ...
    try:
        ...
        events_response = requests.get(f"{HONEYCLOUD_API_URL}/api/events?limit=50", headers=headers, timeout=5)
...
def main():
    ...
    print("   → SSH Honeypot (brute force, command injection)")
    print("   → FTP Honeypot (credential stuffing, file access)")
    print("   → HTTP Honeypot (web probes, admin access)")
    print("   → TELNET Honeypot (network device attacks)")
