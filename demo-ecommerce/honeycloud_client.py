...
Updated for HoneyCloud-X Multi-Tenant API.
"""
...
class HoneyCloudClient:
    ...
    def send_event(
        ...
        # For local demonstration, map loopback/private IPs to random public IPs so they geolocate globally
        if source_ip in ("127.0.0.1", "localhost", "::1") or source_ip.startswith(("192.168.", "10.", "172.16.")):
            import random
            test_ips = [
                "198.51.100.42",   # US
                "95.163.220.12",   # RU
        ...
        # Build payload matching HoneyCloud-X /api/ingest schema
        payload = {
            "service": service,
            "source_ip": source_ip,
            "endpoint": endpoint,
    ...
    def send_failed_login(
        ...
        return self.send_event(
            ...
            endpoint="/login",
            method="POST",
            severity="MEDIUM" if attempt_count < 3 else "HIGH" if attempt_count < 10 else "CRITICAL",
